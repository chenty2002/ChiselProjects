...
module hotel(clk,rst,start,showering,fair);
    ...
    wire [4:0]   reqRegA,reqRegB,reqRegC,reqRegD,reqRegE,
		 reqRegF,reqRegG,reqRegH,reqRegI,reqRegJ;
    ...
    guest GA(clk,rst,start[0],reqInA,S,   grantedA,
	     showering[0],reqOutA,grantA,conditionA,reqRegA);
    guest GB(clk,rst,start[1],reqInB,S,   grantedB,
	     showering[1],reqOutB,grantB,conditionB,reqRegB);
    guest GC(clk,rst,start[2],reqInC,E,   grantedC,
	     showering[2],reqOutC,grantC,conditionC,reqRegC);
    guest GD(clk,rst,start[3],reqInD,SELF,grantedD,
	     showering[3],reqOutD,grantD,conditionD,reqRegD);
    guest GE(clk,rst,start[4],reqInE,N,   grantedE,
	     showering[4],reqOutE,grantE,conditionE,reqRegE);
    guest GF(clk,rst,start[5],reqInF,E,   grantedF,
	     showering[5],reqOutF,grantF,conditionF,reqRegF);
    guest GG(clk,rst,start[6],reqInG,W,   grantedG,
	     showering[6],reqOutG,grantG,conditionG,reqRegG);
    guest GH(clk,rst,start[7],reqInH,S,   grantedH,
	     showering[7],reqOutH,grantH,conditionH,reqRegH);
    guest GI(clk,rst,start[8],reqInI,W,   grantedI,
	     showering[8],reqOutI,grantI,conditionI,reqRegI);
    guest GJ(clk,rst,start[9],reqInJ,S,   grantedJ,
	     showering[9],reqOutJ,grantJ,conditionJ,reqRegJ);

    monitor mtr(clk, conditionA, reqRegA, fair); // Check LTL property.

endmodule // hotel


module guest(clk, reset, start, reqIn, initpred, granted,
	     shower, reqOut, grant, condition, requestReg);
    parameter	 N = 3'd0,
		 W = 3'd1,
		 S = 3'd2,
		 E = 3'd3,
...
//   G(!GAdirty -> X GAreq R (GAreq + !GAshowering)),
// where GAreq is defined as reqOutA[4:0] != 0.
// The Buechi automaton is for the complement of the property,
// which is F(!GAdirty * X !GAreq U (!GAreq * GAshowering)).
module monitor(clk,condition,req,fair);
    input       clk;
    input [1:0] condition;
    input [4:0] req;
    output 	fair;
    ...
    assign fair = (state == double);

    initial state = start;

    always @ (posedge clk) begin
	case (state)
	  start: state = req != 0 ? gotreq :
			 condition == 2'b10 ? double : start;
	  gotreq: state = condition == 2'b10 ? start : gotreq;
	  double: state = double;
	endcase // case(state)
    end // always @ (posedge clk)
