...
AG(adFld1[1:0]==adFld2[1:0] -> (State[2:1]=0 + A[4:0]==B[4:0]));

#PASS: In case of branch, Cond is consistent with A in the MEM and WB states.
AG(branch=1 * State[2]=1 -> ((Cond=1 -> A[4:0]=0) * (A[4:0]=0 -> Cond=1)));

#PASS: a XOR a = 0.
AG(regRegALU=1 * funFld[2:0]=b100 * adFld1[1:0]==adFld2[1:0] ->
   (State[2]=1 -> ALUOutput[4:0]=0));

#PASS: XOR is performed correctly.
AG(regRegALU=1 * funFld[2:0]=b100 * 
...
AG(regRegALU=1 * funFld[2:1]=b01 * adFld1[1:0]==adFld2[1:0] ->
   (State[2]=1 -> ALUOutput[4:0]==A[4:0]));

#PASS: AND is performed correctly.
AG(regRegALU=1 * funFld[2:0]=b010 * 
   (State[2]=1 + State[1:0]=0 + State[1:0]=3) ->
...
AG(regRegALU=1 * funFld[2:0]=b011 * 
   (State[2]=1 + State[1:0]=0 + State[1:0]=3) ->
...
#    A[4:0]=1 * B[4:0]=2);
AG(\R0eqR1 + \R0eqR2 + \R0eqR3 + \R1eqR2 + \R1eqR3 + \R2eqR3 + \AeqR0 +
...
AG(\IF *
   ...
   AX:5(Registers<*1*>[4:0]=1 * Registers<*2*>[4:0]=2 *
